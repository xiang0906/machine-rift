...
- V15 會為 `game_record` 加入 `created_at`，讓新舊戰績都有可供歷史紀錄排序與顯示的日期。
- V16 會恢復 `player_tower_unlock` 玩家與塔的解鎖關聯，將 V14 的解鎖數量安全搬回
...
- V19 移除僅供展示、未參與玩法的玩家等級與經驗值欄位，保留實際影響遊戲的進度資料。
- V20 小幅提高後期耐久型敵人的移動速度，改善後段波次節奏，並維持前三種前期敵人速度不變。
- V21 將疾風無人機以外的五種敵人跑速各提高 10，縮短中後段等待時間。
...
- 核心資料表：`player`、`player_stage_progress`、`player_tower_unlock`、`stage`、
  `stage_path`、`stage_wave`、`tower`、`enemy`、`game_record`。資料表之間以外鍵維持玩家、
  關卡與戰績的關聯完整性。
...
| 塔 | GET | `/towers` | 取得所有塔 |
| 塔 | POST | `/towers/{towerId}/unlock` | 使用永久戰備金解鎖指定防禦塔（需登入） |
| 戰績 | POST | `/game-records` | 儲存本人的對戰紀錄（需登入） |
| 戰績 | GET | `/game-records/me` | 取得本人最近 20 場歷史戰績（需登入） |
| 排行榜 | GET | `/rankings?stageId={id}` | 取得全部或指定關卡的前十名排行榜（需登入；`stageId` 可省略） |

目前共保留 11 支核心 API；未被遊戲流程使用的公開玩家清單、單筆查詢與後台 CRUD
端點已移除。防禦塔購買 API 已由遊戲大廳中的防禦塔工坊使用。
...
七個畫面使用單頁 hash 路由，各自對應 `#/login`、`#/register`、`#/lobby`、`#/towers`、
`#/stages`、`#/records` 與 `#/game`。舊的 `#/history`、`#/ranking` 仍可開啟，並會導向戰績中心。
切換畫面時網址會同步更新，也支援瀏覽器上一頁／下一頁。大廳、工坊、關卡、戰績中心與遊戲頁需要登入；
直接開啟受保護網址時會回到登入頁。戰鬥狀態只存在
...
2. 前端保存 access token；重新整理或再次開啟時以 `GET /api/auth/me` 恢復同一玩家，
   並以 `GET /api/players/me/progress` 取得該玩家進度後進入遊戲大廳
3. 大廳顯示永久戰備金、完成關卡與解鎖塔數，玩家可自由前往工坊、任務或戰績中心
4. 防禦塔工坊 → `GET /api/towers` 顯示全部塔種，以 `POST /api/towers/{towerId}/unlock` 永久解鎖
5. 選擇關卡 → `GET /api/stages` 讀取關卡清單並顯示靜態任務簡報
6. 建塔 → 使用玩家已解鎖的塔，點選塔種後點地圖空格建造
7. 開始波次 → 依資料庫波次與敵人設定生成敵人
8. 結束遊戲 → 顯示勝敗敘事，並帶著登入 token 呼叫 `POST /api/game-records`
9. 戰績中心的個人歷史 → `GET /api/game-records/me` 取得最近 20 場紀錄與日期
10. 戰績中心的榮耀排行榜 → `GET /api/rankings` 或 `?stageId={id}` 取得全部或指定關卡前十名
...
新玩家會自動取得第一關與最便宜的起始防塔。每次儲存戰績時，後端會更新永久戰備金、
通關次數及該關最佳分數／時間。首次通關會解鎖下一關；勝利
不再自動解鎖防禦塔。玩家可在工坊查看全部塔種、戰鬥數值、戰場造價與永久解鎖價格，
再使用永久戰備金自由購買；購買成功後大廳與遊戲塔庫會立即同步。前端會顯示所有關卡，
未解鎖關卡則停用進入按鈕。

登入後的大廳採用裂隙作戰指揮中心介面，集中顯示指揮官資料、任務與防塔解鎖進度，
並依既有關卡進度與戰備金提供下一步建議。任務、工坊與戰績入口使用不同識別色與圖示，
所有內容均由既有前端狀態產生，不增加 API 或資料表。

整體介面採用科幻戰術藍圖風格：深藍網格與青灰細線作為基礎，青色保留給任務、波次與
選取狀態，黃色代表金幣與資源，綠色代表成功與安全，紅色代表危險與失敗。卡片使用霧面
面板、角落戰術標記與較低光暈；Canvas 戰場包含低對比地形分區及間歇掃描線。

觸控手機以直式進入戰場時，系統會先提示玩家將裝置旋轉為橫向；旋轉完成後會自動部署，
也可選擇維持直式進入或返回任務簡報。桌面與已處於橫向的手機不顯示此提示。

工坊、關卡選擇與戰績中心共用一致的頁首結構：英文分類標籤、中文主標題、用途說明，
以及位於標題區右上方的返回大廳按鈕；手機版則將返回按鈕放在頁首文字下方並填滿寬度。
...
建造位置是否合法並預覽攻擊範圍；金幣足夠時會保留目前塔種，可連續建造同一座塔，
剩餘金幣不足以再建造時才會自動取消選取。
戰場頁首會以任務名稱與波次作為第一層資訊；HUD 將作戰時間、生成進度、基地耐久與分數
整理為一致的狀態卡。波次切換只播放一次短動畫，低血量才持續警示，操作訊息則以一般、
資訊、成功、警告與危險層級顯示。
...
前端可選擇全部關卡或任一關卡，並顯示該範圍的前三名頒獎台及前十名列表。

戰績中心入口位於遊戲大廳，以頁籤整合個人歷史與榮耀排行榜。個人歷史只查詢目前
Token 對應玩家最近 20 場紀錄，並依建立日期由新到舊顯示關卡、勝敗、分數與遊戲時間，
不公開其他玩家的完整紀錄。

登入或恢復 Session 後會先進入 `#/lobby` 遊戲大廳。大廳使用既有玩家進度資料呈現
玩家摘要，並集中提供工坊、任務、戰績中心與登出入口；關卡選擇頁只保留關卡與
操作教學，所有次要功能頁統一返回大廳。

## 專案現況
- 後端已保留遊戲核心流程需要的 11 支 API，並提供集中式例外處理。
- 玩家帳號、加密密碼、持久登入、個人進度、解鎖內容與最佳戰績已完成。
- 關卡路徑、波次與敵人設定皆由資料庫及 API 提供。
- 遊戲內容目前包含六個關卡、六種防塔、六種敵人與 26 個波次。
- 前端已有可玩的塔防流程，並串接登入、防禦塔工坊、關卡／塔資料、玩家進度、個人歷史戰績與排行榜。
