...
目前共保留 10 支前端實際使用的 API；未被遊戲流程使用的公開玩家清單、單筆查詢與
後台 CRUD 端點已移除。
...
六個畫面使用單頁 hash 路由，各自對應 `#/login`、`#/register`、`#/stages`、
`#/history`、`#/ranking` 與 `#/game`。切換畫面時網址會同步更新，也支援瀏覽器上一頁／下一頁。
關卡、個人戰績、排行榜與遊戲頁需要登入；直接開啟受保護網址時會回到登入頁。戰鬥狀態只存在
...
7. 在關卡選擇頁可查看個人歷史 → `GET /api/game-records/me` 取得最近 20 場紀錄與日期
8. 登入後可另外查看排行榜 → `GET /api/rankings` 直接取得統計與前十名
...
個人歷史戰績入口位於關卡選擇頁，只查詢目前 Token 對應玩家最近 20 場紀錄，並依
建立日期由新到舊顯示關卡、勝敗、分數與遊戲時間，不公開其他玩家的完整紀錄。
...
- 前端已有可玩的塔防流程，並串接登入、關卡／塔資料、玩家進度、個人歷史戰績與排行榜。
