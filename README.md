...
- 核心資料表：`player`、`player_session`、`player_progress`、`player_stage_progress`、
  `player_tower_unlock`、`stage`、`tower`、`game_record`。刪除 `player` 或 `stage`
  時，若已有關聯的 `game_record`，會被拒絕（回傳 409 衝突）。
...
7. 可另外查看排行榜 → `GET /api/rankings` 搭配 `GET /api/players` 顯示玩家名稱
...
玩家等級、金幣、通關次數及該關最佳分數／時間；首次通關會依序解鎖下一關與下一座
防塔。前端會顯示所有關卡，未解鎖關卡則停用進入按鈕。

## 專案現況
- 後端 API 骨架與資料完整性防護已完成（玩家/關卡刪除保護、集中式例外處理）。
- 玩家帳號、加密密碼、持久登入、個人進度、解鎖內容與最佳戰績已完成。
- 關卡路徑、波次與敵人設定皆由資料庫及 API 提供。
- 前端已有可玩的最小塔防切片，並串接登入、關卡／塔資料、戰績與排行榜。
- 尚未做的：密碼重設、電子郵件驗證、塔升級機制與更多關卡內容。
